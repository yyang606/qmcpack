...
int main(int argc, char **argv)
{
  ...
  // read sampled 1D S(k)
  xmlNodePtr splk_node = find("//splk", doc);
  xmlNodePtr splsk_node = find("//splsk", doc);
  vector<RealType> splk, splsk;
  putContent(splk, splk_node);
  ...
  for (int ik=0; ik<splk.size(); ik++)
  {
    app_log() << splk[ik] << " " << splsk[ik] << endl;
  }
  app_log() << "#SPLSK_STOP#" << endl;
  app_log() << "#FINESK_START#" << endl;
  for (int ik=0; ik<nk; ik++)
  {
    RealType kmag = ik*dk;
    RealType skval;
    eval_NUBspline_1d_d(fsplsk, kmag, &skval);
    app_log() << kmag << " " << skval << endl;
  }

...
public:
  ...
  void resize(const std::vector<PosType>& kin, const std::map<int, int> kmap_in, const int Min, const std::vector<RealType>& rmags);
  ///number of samples
  int M;
  ...
  Matrix<RealType> rij;
  ///fofr temporary data
  std::vector<RealType> rmags;
  std::vector<RealType> fofr;
};

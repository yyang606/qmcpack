...
{ 
  ...
  if (hdf5_out)
  {
    // put this variable in stat.h5 instead of scalar.dat
...
{ 
  SpeciesKineticEnergy* myclone = new SpeciesKineticEnergy(*this);
  myclone->hdf5_out = hdf5_out;
  myclone->h5_index = h5_index;
  copy( vec_minus_over_2m.begin(),vec_minus_over_2m.end(),myclone->vec_minus_over_2m.begin() );
  return myclone;
}
